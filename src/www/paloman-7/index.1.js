...
codigo_actual:"\nhago entrar.escena(\"escena_inicial\").activarse().\nhago entrar.objeto_esfera(\"esfera_inicial\", \"Objeto_esfera_estandar\", [{ diameter: 2, segments: 32 }], una lambda con (esfera) donde asigno esfera.$mesh.position.y como 1).\nhago entrar.objeto_suelo(\"suelo_inicial\", \"Objeto_suelo_estandar\", [{ width: 6, height: 6 }]).\nhago entrar.persona(\"persona_inicial\", \"Persona_estandar\", [{}, {}, this.escena_actual.$scene]).\nhago entrar.camara(\"camara_inicial\", \"Camara_estandar\", [], una lambda con (camara) donde hago camara.$camera.setTarget(objeto(\"esfera_inicial\").$mesh)).\nhago entrar.camara(\"camara_perfil\").\nhago entrar.camara(\"camara_frontal\").\nhago entrar.luz(\"luz_inicial\").\nhago camara(\"camara_inicial\").moverse({ x: 5, y: 0, z: 0 }).\nhago objeto(\"esfera_inicial\").moverse({ x: 1.2, y: 0, z: 0 }).\nhago persona(\"persona_inicial\").rotar.eje({ x: 20, y: 0, z: 0 }).\nhago persona(\"persona_inicial\").rotar.eje({ x: 20, y: 0, z: 0 }).\nhago entrar.evento_en_bucle(\"mover esfera inicial\", una lambda con () donde hago objeto(\"esfera_inicial\").moverse({ x: 0.001, y: 0, z: 0 })).\nasigno window.$h como this.\nretorno escena(\"escena_inicial\").$scene.\n".trim(  ),
...
const codigo_temporal = `((homactografo, componente) => homactografo.representar(function async () { try { const {entrar,salir,elemento,elementos,escena,escenas,camara,camaras,luz,luces,persona,personas,objeto,objetos} = homactografo.selectores(); ${codigo_js} } catch(error) { componente.mostrar_error(error); } }))`;
